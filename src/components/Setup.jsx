...
export default function Setup({ onCreate, loadingCountdown }) {
  ...
  useEffect(() => {
    const wanted = Math.max(2, Math.min(20, Number(count || 0)))
    setPlayers((prev) => {
      const next = [...prev]
  ...
  return (
    <div className={styles.container}>
      <header style={{ marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 14, color: '#cfe6c9', marginTop: 4 }}>Setup del tavolo</div>
      </header>

      <form className={styles.form} onSubmit={handleSubmit}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 12 }}>
          <label className={styles.row} style={{ margin: 0 }}>
